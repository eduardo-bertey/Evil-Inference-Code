"""Test de convergencia: laurelia con 4 capas, BPE 16k, seq 2k, batch 8.
Entrena sobre los 50MB de wiki ES. Al final guarda train_history.json y
plot_test_laurelia.png.
"""

import sys, os, time, json, math, torch

_DIR = os.path.dirname(os.path.abspath(__file__))
_FILO = os.path.join(_DIR, "..", "laurelia-llm")
...
def main():
    ...
    plot_path = os.path.join(_DIR, "plot_test_laurelia.png")
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        plt.xlabel("step")
        plt.ylabel("loss")
        plt.title(f"Test laurelia convergencia ({config.layers} capas, seq {config.block_size})")
    ...
    for f in [hist_path, plot_path]:
        api.upload_file(
            ...
            revision="doc-llm",
            commit_message=f"test_laurelia step {step}",
        )
