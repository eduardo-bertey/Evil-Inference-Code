...
def main():
    ...
    plot_path = os.path.join(_DIR, "plot_test_dlorai.png")
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        plt.xlabel("step")
        plt.ylabel("loss")
        plt.title(f"Test dlorai convergencia ({config.layers} capas, seq {config.block_size})")
    ...
    for f in [hist_path, plot_path]:
        api.upload_file(
            ...
            revision="doc-llm",
            commit_message=f"test_dlorai step {step}",
        )
