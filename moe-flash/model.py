...
Features:
  ...
  - MoE with bmm experts, bias trick, z-loss, capacity (no shared experts)
...
from xkv_cache import XKVConfig, XVKSRCoordinator, make_default_config
...
class TransformerLM(nn.Module):
    ...
    def __init__(
        ...
        n_dense_start: int = 3,
        n_dense_end: int = 3,
        # xKV-SR (cross-layer SVD KV + selective reconstruction, MLA)
        use_xkv: bool = False,
        ...
        if use_xkv:
            xkv_cfg = XKVConfig(
                group_size=xkv_group_size,
                rank_k=xkv_rank_k,
                rank_v=xkv_rank_v,
                sparse_budget=xkv_sparse_budget,
        ...
        # xKV coordinator attached lazily (needs device/dtype after .to())
        if use_xkv and use_mla:
            self._xkv_pending = True
        else:
            self._xkv_pending = False

    def setup_xkv(self, device=None, dtype=None) -> XVKSRCoordinator:
        """Create shared XVKSRCoordinator and bind it to every MLA layer.

        Safe to call multiple times; no-op if already set up or xKV disabled.
        """
        if self.xkv_cfg is None or not self.use_mla:
            raise RuntimeError("setup_xkv: model was not built with use_xkv + use_mla")
        if self.xkv_coord is not None and not self._xkv_pending:
            return self.xkv_coord
        ...
        self.xkv_coord = XVKSRCoordinator(
            cfg=self.xkv_cfg,
        ...
        for layer in self.transformer.layers:
            ...
            if hasattr(attn, "xkv_coord"):
                attn.xkv_coord = self.xkv_coord
        self._xkv_pending = False
        return self.xkv_coord

    def _ensure_xkv(self):
        if self._xkv_pending:
            self.setup_xkv()
