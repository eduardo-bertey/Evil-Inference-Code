...
# ─── xKV-SR (cross-layer SVD KV + selective reconstruction) ─────────────────
use_xkv = True
xkv_group_size = 4      # capas por grupo SVD
xkv_rank_k = 128        # rank SVD keys (latent MLA sigue en 64)
xkv_rank_v = 128        # rank SVD values
...
# n_experts = [5, 5, 5, 6, 6, 6, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 6, 6, 6, 5, 5, 5, 5]
...
def main():
    ...
    model = TransformerLM(
        ...
        use_xkv=use_xkv,
        xkv_group_size=xkv_group_size,
        xkv_rank_k=xkv_rank_k,
        xkv_rank_v=xkv_rank_v,
    ...
    if use_moe:
        ...
        print(f"MoE{ntk_tag}: {moe_layers}/{num_layers} MoE layers | {n_dense_start} dense start / {n_dense_end} dense end | n_exp={n_experts} top_k={top_k}")
    if use_xkv and getattr(model, "xkv_cfg", None):
        c = model.xkv_cfg
        print(f"xKV-SR: group={c.group_size} rank_k={c.rank_k} rank_v={c.rank_v} "
              f"budget={c.sparse_budget} chunk={c.chunk_size} local={c.local_window} "
              f"| latent MLA d_c={mla_d_c} (sin comprimir K_rot/RoPE)")
