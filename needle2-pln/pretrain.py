"""Needle2 PLN Train — SimpleAttentionNetwork (puerto PyTorch de cactus needle v2).

bf16/f16/f32, dataset streaming en bloques de 6MB (wiki 3 + fine 2 + tuit 1),
AdamW fused, WSD schedule, HF upload.
Branch HF: needle (repo ScortexIA/laurelia).
"""
...
def main():
    ...
    if test_mode:
        ...
        prec = input("Precision (n=f32, h=f16, b=bf16): ").strip().lower()
        dtype = {"b": torch.bfloat16, "h": torch.float16}.get(prec, torch.float32)
    ...
    mhc_p = sum(p.numel() for k, p in model.named_parameters() if k.startswith(("phi_", "b_", "a_")))
    eng_p = sum(p.numel() for e in model.engrams for p in e.parameters())
    print(f"Params: emb={emb_p:,} + {config.layers}capas={layer_p:,} + mhc={mhc_p:,} + engram={eng_p:,}"
          f" = {sum(p.numel() for p in model.parameters()):,}")
    print(f"dim={config.dim} lay={config.layers} heads={config.heads} kv={config.kv_groups}"
          f" lanes={config.mhc_lanes} engram={config.engram_layers} window={config.kv_window}"
          f" seq={seq_len} bs={config.batch_size} ga={config.grad_acc} lr={config.learning_rate}")
    print(f"Tokens: {n:,}")
            ...
            if (batch_start // config.batch_size + 1) % config.grad_acc == 0 or batch_end >= n_seq:
                ...
                if not test_mode and pusher and (time.time() - pusher.last_push) >= pusher.interval:
                    ...
                    ckpt = {"step": step, "epoch": epoch,
                            "block": sd.block_idx if not test_mode else 0,
                            "config": vars(config), "model": state}
