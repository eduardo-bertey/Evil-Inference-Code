"""MLKV Train — Multi-Layer KV sharing sobre base laurelia-llm.

bf16, streaming dataset, AdamW fused, WSD schedule, HF upload.
Branch HF: mlkv (repo ScortexIA/laurelia).
"""
...
from model import LLM, Config, kv_cache_bytes
import importlib
...
def main():
    ...
    # Override de grupos por CLI: python train.py --kv 4
    if "--kv" in sys.argv:
        idx = sys.argv.index("--kv")
        config.num_kv_layers = int(sys.argv[idx + 1])
    ...
    kv_dense = kv_cache_bytes(config, seq_len)
    ...
    print(f"dim={config.dim} lay={config.layers} heads={config.heads} kv={config.kv_groups} "
          f"grupos={config.num_kv_layers}x{config.layers // config.num_kv_layers} "
          f"seq={seq_len} bs={config.batch_size} ga={config.grad_acc} lr={config.learning_rate}")
    print(f"KV cache @{seq_len}: {kv_dense/1024:.0f}KB/token-lote "
          f"(dense sería {kv_dense * config.layers / config.num_kv_layers /1024:.0f}KB)")
    print(f"Tokens: {n:,}")
            ...
            if (batch_start // config.batch_size + 1) % config.grad_acc == 0 or batch_end >= n_seq:
                ...
                if not test_mode and pusher and (time.time() - pusher.last_push) >= pusher.interval:
                    ...
                    ckpt = {"step": step, "epoch": epoch,
                            ...
                            "num_kv_layers": config.num_kv_layers,
                            "model": state}
