"""TransformerLM — GQA con K=V compartida + MoSE, basado en LLM_350M_DENSE.

Atencion: sin k_proj; K = V (scores Q contra V rotado, values sin rotar).
Cache solo-V (K no se guarda). FFN = MoSE (slimmable experts 25/50/75/100%).
Init adaptativo por capa, weight tying, cache para inferencia.
"""
...
class Config:
    ...
    # MoSE en el FFN: 4 expertos slimmable, anchos 25/50/75/100%.
    mose_expertos = 4
    mose_widths = (0.25, 0.50, 0.75, 1.0)
    mose_aux_w = 0.01  # peso del balanceo experto+ancho
...
class Attention(nn.Module):
    """K=V compartida: sin k_proj, K no se guarda.
    Q rotado con RoPE, K = RoPE(V) solo para scores, V sin rotar.
    Cache = solo V."""
    ...
    def forward(self, x):
        ...
        q, k = self.rope(q, v, 0)  # K = V rotado; v intacto para values
    ...
    def forward_with_cache(self, x, offset, cache):
        ...
        q_new, _ = self.rope(q_new, v_new, offset)
        ...
        new_cache = v_full.clone()  # solo V, sin rotar
...
class Block(nn.Module):
    def __init__(self, config):
        ...
        self.mlp = MoSE(config)  # FFN = Mixture of Slimmable Experts

    def forward(self, x, width=None):
        x = x + self.attn(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x), width)
        return x
    ...
    def forward_with_cache(self, x, offset, cache):
        ...
        x = x + h
        x = x + self.mlp(self.ln_2(x))  # inferencia: el router elige experto+ancho
        return x, new_cache
...
class LLM(nn.Module):
    ...
    def forward(self, input_ids, labels=None, width=None):
        # Train: cada forward muestrea un ancho para entrenar todas las dims.
        if width is None and self.training:
            width = MoSE.sample_width(self.config.mose_widths)
    ...
    def aux_total(self):
        """Suma last_aux de todos los MoSE (0.0 si no hay)."""
